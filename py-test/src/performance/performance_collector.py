  ...
  End: {report.end_time.isoformat()}
    ...
    def check_slo(self, slo: PerformanceSLO) -> SLOResult:
        ...
        if report.p95_latency > slo.max_p95_latency:
            violations.append(
                f"P95 latency {report.p95_latency:.0f}ms exceeds SLO {slo.max_p95_latency}ms"
            )
        ...
        if report.error_rate > slo.max_error_rate:
            violations.append(
                f"Error rate {report.error_rate * 100:.1f}% exceeds SLO {slo.max_error_rate * 100:.1f}%"
            )

        if slo.min_throughput and report.requests_per_second < slo.min_throughput:
            violations.append(
                f"Throughput {report.requests_per_second:.1f} req/s below SLO {slo.min_throughput} req/s"
            )
