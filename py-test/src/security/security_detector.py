...
PROMPT_INJECTION_PATTERNS = [
    re.compile(r'ignore\s+(?:all\s+)?(?:previous|above|prior)\s+(?:instructions|prompts|commands)', re.I),
    ...
    re.compile(r'act\s+as\s+(?:if\s+)?(?:you\s+are|you\'re)\s+(?:a|an)\s+\w+', re.I),
    re.compile(r'what\s+(?:are|were|is)\s+your\s+(?:original|initial|system)\s+(?:instructions|prompt|rules)', re.I),
    ...
    def analyze_input(self, input_text: str) -> SecurityDetectionResult:
        ...
        if len(input_text) > MAX_INPUT_LENGTH:
            violations.append(SecurityViolation(
                ...
                severity='high',
                description=f'Input exceeds maximum length ({MAX_INPUT_LENGTH} characters) - potential DoS/ReDoS attack',
    ...
    def analyze_output(self, output_text: str) -> SecurityDetectionResult:
        ...
        if len(output_text) > MAX_OUTPUT_LENGTH:
            violations.append(SecurityViolation(
                type='data_exfiltration',
                severity='medium',
                description=f'Output exceeds maximum length ({MAX_OUTPUT_LENGTH} characters) - potential data exfiltration',
            ...
            if matches:
                ...
                for match in matches:
                    violations.append(SecurityViolation(
                        type='pii_leakage',
                        severity=severity,
                        description=f'{self._format_pii_type(pii_type)} detected in output',
                        matched_pattern=self._mask_sensitive_data(match.group(0)),
                        location='output',
                        recommendation=f'Remove or redact {self._format_pii_type(pii_type)} before returning to user.'
                    ))
