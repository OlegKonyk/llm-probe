...
class SummaryEvaluator:
    ...
    def generate_report(self, summary: str, test_case: GoldenTestCase) -> str:
        ...
        report += "Metrics:\n"
        report += f"  Similarity: {result.similarity:.3f} (threshold: {test_case.thresholds['min_semantic_similarity']})\n"
        report += f"  BLEU Score: {result.bleu:.3f}\n"
        report += f"  Word Count: {result.length_check['word_count']} (range: [{test_case.thresholds['min_length_words']}, {test_case.thresholds['max_length_words']}])\n"
        report += f"  Required Terms Coverage: {result.required_terms['coverage'] * 100:.0f}%\n"
