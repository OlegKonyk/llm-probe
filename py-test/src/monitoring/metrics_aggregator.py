    ...
    def check_for_regressions(self) -> list[RegressionAlert]:
        ...
        if latest.avg_similarity is not None:
            ...
            if sim_change < -self.thresholds.similarity_drop:
                alerts.append(RegressionAlert(
                    severity='critical' if sim_change <= -CRITICAL_SIMILARITY_DROP_THRESHOLD else 'warning',
                    metric='similarity',
                    current=latest.avg_similarity,
                    baseline=baseline.avg_similarity,
                    percent_change=sim_change * 100,
                    message=f"Similarity dropped {abs(sim_change * 100):.1f}% ({latest.avg_similarity:.2f} vs baseline {baseline.avg_similarity:.2f})",
        ...
        if latest.avg_latency is not None and baseline.avg_latency > 0:
            ...
            if latency_change > self.thresholds.latency_increase:
                alerts.append(RegressionAlert(
                    severity='critical' if latency_change >= CRITICAL_LATENCY_INCREASE_THRESHOLD else 'warning',
                    metric='latency',
                    current=latest.avg_latency,
                    baseline=baseline.avg_latency,
                    percent_change=latency_change * 100,
                    message=f"Latency increased {latency_change * 100:.1f}% ({latest.avg_latency:.0f}ms vs baseline {baseline.avg_latency:.0f}ms)",
                    timestamp=latest.timestamp
                ))
        ...
        if pass_rate_change < -self.thresholds.pass_rate_drop:
            alerts.append(RegressionAlert(
                severity='critical' if pass_rate_change < -CRITICAL_PASS_RATE_DROP_THRESHOLD else 'warning',
                metric='pass_rate',
                current=current_pass_rate,
                baseline=baseline.avg_pass_rate,
                percent_change=(pass_rate_change / baseline.avg_pass_rate) * 100 if baseline.avg_pass_rate > 0 else 0.0,
                message=f"Pass rate dropped {abs(pass_rate_change * 100):.1f}% ({current_pass_rate * 100:.1f}% vs baseline {baseline.avg_pass_rate * 100:.1f}%)",
                timestamp=latest.timestamp
            ))
...
Current Status:
  Tests: {latest.passed}/{latest.total_tests} passed ({latest.passed / latest.total_tests * 100:.1f}%)
  Duration: {latest.duration / 1000:.1f}s
"""
...
"""
    ...
    def export_prometheus_metrics(self) -> str:
        ...
        if latest.avg_similarity is not None:
            ...
            metrics.append(f'llm_similarity_score{{suite="{latest.test_suite}"}} {latest.avg_similarity}')
