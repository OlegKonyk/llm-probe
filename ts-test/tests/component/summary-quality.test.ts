/**
 * Summary Quality Component Tests
 *
 * Tests the full evaluation pipeline: metrics + thresholds + pass/fail logic.
 * These are component tests (not unit tests) because they test multiple
 * integrated components working together.
 *
 * What We're Testing:
 * - End-to-end quality evaluation against golden dataset
 * - Pass/fail logic with real test cases
 * - Failure detection for common quality issues:
 *   - Too short/long summaries
 ...
 * - call_001 (easy): Password reset with email mismatch
 * - call_002 (medium): Billing inquiry with overage and plan upgrade
 * - call_003 (hard): Product issue with troubleshooting and refund
 * - call_004 (easy): Simple account address update
 * - call_005 (medium): General inquiry about plan differences
 *
 * Test Pattern:
 * These tests use the real golden dataset but mock LLM outputs.
 * They validate that our evaluation logic correctly identifies
 * good vs bad summaries.
 *
 * No LLM Required:
 * These tests run without a live LLM, making them fast and reliable
 * for CI/CD. The LLM outputs are simulated/hard-coded.
 *
 * Relationship to Other Tests:
 * - Unit tests: Test individual metrics in isolation
 * - Component tests (this file): Test evaluation pipeline
 * - Integration tests: Test with real API/LLM
...
describe('Summary Quality Component Tests', () => {
  ...
  describe('Password Reset Call (call_001)', () => {
    /**
     * Test: High-Quality Summary Should Pass
     *
     * Validates that a well-written summary meeting all criteria passes:
     * - High semantic similarity (0.80+)
     * - Appropriate length (30-100 words)
    ...
    it('should pass evaluation with high-quality summary', () => {
      ...
      // Simulate a good LLM-generated summary
      const summary =
    ...
    it('should fail evaluation if summary is too short', () => {
      ...
      const summary = 'Password reset issue fixed.'; // Too short
  ...
  describe('Billing Inquiry Call (call_002)', () => {
    it('should pass evaluation with comprehensive summary', () => {
      ...
      // Check individual metrics even if overall pass fails
    ...
    it('should detect missing key information', () => {
      ...
      // Should fail due to low similarity (missing important details)
      expect(result.similarity).toBeLessThan(
        testCase.thresholds.min_semantic_similarity
      );
    });
  ...
  describe('Product Issue Call (call_003)', () => {
    it('should handle negative sentiment appropriately', () => {
      ...
      expect(result.similarity).toBeGreaterThan(0.75);
      expect(result.requiredTerms.coverage).toBe(1.0);
      expect(result.lengthCheck.passed).toBe(true);
    });
  });
  ...
  describe('Consistency Evaluation', () => {
    it('should detect high consistency between similar summaries', () => {
      ...
      // Note: Simple lexical similarity is lower than semantic embeddings would be
      // These summaries are semantically similar but use different words
  ...
  describe('Golden Dataset Coverage', () => {
    it('should evaluate all test cases in golden dataset', () => {
      ...
      allCases.forEach((testCase) => {
        ...
        expect(result.requiredTerms.coverage).toBeGreaterThan(0.5);
      });
    });
    ...
    it('should load and evaluate each category', () => {
      ...
      categories.forEach((category) => {
        ...
        cases.forEach((testCase) => {
          // Use golden summary as a "perfect" example
          const result = evaluator.evaluate(
            testCase.golden_summary,
            testCase
          );
